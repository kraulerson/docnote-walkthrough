...
- **Never-do rules (each is a review-blocking defect):**
  1. Never assign document/user-influenced strings to `innerHTML`/`outerHTML`/`insertAdjacentHTML`/jQuery-`.html()` — the ONLY sanctioned sink is `sanitizeToFragment()` in `src/core/sanitize.ts` (DOMPurify inside).
