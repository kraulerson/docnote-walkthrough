...
| 14 | SEV-3 | Fixed | highlight-apply | Keyboard-only text selection never surfaces the highlight toolbar (mouseup-only trigger). PARTIAL: added a keyup handler + a focusable (tabIndex=0) document region so a keyboard selection surfaces the toolbar; full caret-based keyboard selection depends on the browser's caret-browsing mode (documented in USER_GUIDE). Flagged by the Phase 3 senior-engineer review as previously over-claimed. | Session 1 | Phase 3 accessibility (partial) | `e05c494` | Phase 3 |
...
| 24 | SEV-3 | Fixed | notes-crud | Esc closes neither HighlightMenu nor NoteEditor and neither traps focus (Bible §9/§14). DONE: Escape now closes the menu/editor/toolbar; the note editor autofocuses its textarea. NOT DONE: a full focus-trap (Tab cycling within the popover) — accepted for MVP. Flagged by the Phase 3 senior-engineer review as previously over-claimed on the focus-trap part. | Session 2 | Phase 3 accessibility (partial) | `e05c494` | Phase 3 |
...
| 32 | SEV-1 | Fixed | docx-open-render | RT-01: the BUG-1 decompression-bomb guard trusts the ADVERTISED ZIP central-directory uncompressed sizes — a `.docx` that LIES (under-reports the size) passes the guard while JSZip/mammoth still inflates the real payload with no cap, re-opening the SEV-1 DoS (multi-GB inflation / tab freeze). The "bounded by 10 MB compressed" reasoning was wrong: 10 MB of deflate inflates to multi-GB. Security review rated this Low (F-1); the Red Team review correctly escalated it to a ship-blocker under the project's own SEV-1/TM-007 policy — proven with a probe (honest zip→guard fires, lying zip→guard misses, JSZip inflates fully). | Phase 3 review (security F-1 + red-team RT-01) | Fix Now | | |
| 33 | SEV-4 | Won't Fix | docx-open-render | meta-CSP cannot deliver `frame-ancestors` (header-only), leaving a clickjacking residual on GitHub Pages (which can't set response headers). Already disclosed in Bible §4 / threat-model validation (security review F-2, Low). | Phase 3 review | Won't Fix (accepted/disclosed; set `frame-ancestors` at the deploy layer if moved to a host that supports response headers) | | |
<!--
