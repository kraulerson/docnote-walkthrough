...
| **TM-007** | DoS | `src/core/zipGuard.ts` (advertised + ACTUAL-inflation guards) + `src/core/parseDocx.ts` caps | **PASS (hardened after red-team RT-01)** | Two-layer decompression-bomb guard: (1) fast reject on advertised central-directory size (honest bombs), and (2) **bounded actual inflation via `DecompressionStream` with early abort** — a ZIP that LIES about its size can no longer slip a bomb past (RT-01/BUG-32 fix). Both the honest bomb and the lying bomb fixtures are rejected as file-too-large (zipGuard.test.ts, parseDocx.test.ts). Plus 10 MB file cap + 5M-char extracted cap. Residual (main-thread inflation, bounded to the budget) accepted for MVP; a Web Worker watchdog is the future enhancement. |
