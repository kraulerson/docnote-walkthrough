...
| 32 | SEV-4 | Won't Fix | docx-open-render | Decompression-bomb guard trusts the ADVERTISED ZIP central-directory uncompressed sizes; a hostile ZIP that under-reports them could evade the guard and cause a recoverable main-thread freeze — bounded by the 10 MB compressed-file cap (found by the Phase 3 security review, F-1, Low). | Phase 3 review | Won't Fix (accepted: bounded by 10 MB cap; worker-based parse is the TM-007 future mitigation) | | |
