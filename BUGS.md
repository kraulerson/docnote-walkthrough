...
| 27 | SEV-3 | Open | local-persistence | restoredForHash ref is dead code; the save effect fires on the first post-restore render and rewrites the store with createdAt=now, so AnnotationStore.createdAt (Bible §5 = creation time) never survives a reload; save-on-open can also mis-attribute a "storage full" warning. | Session 3 | Fix Now | | |
| 28 | SEV-3 | Open | local-persistence | loadAnnotations returns the raw JSON.parse object graph; the Bible §4 vuln#3 / TM-003 storage-side control ("validate into fresh typed structures") is absent — tampered extra fields round-trip into state and back to storage. Not exploitable today (sinks are textContent), a defense-in-depth gap. | Session 3 | Fix Now | | |
| 29 | SEV-4 | Open | local-persistence | Restore validation looser than the data model: accepts empty-string notes, unbounded note length, and negative/huge/fractional anchor offsets. Not exploitable (downstream re-checks → unlocated, no crash). | Session 3 | Fix Now (fold into BUG-28 reconstruction) | | |
| 30 | SEV-3 | Open | docx-open-render | parseDocx computes fullText/paragraphCount with the naive selector, not the leaf-only filter getBlockElements got for BUG-2 — so docHash double-counts nested blocks (td>p, li>ul>li). Deterministic (identity stable) but inconsistent with the anchoring model. BUG-2 completeness. | Session 3 | Fix Now | | |
| 31 | SEV-4 | Open | notes-panel-jump | NotesPanel omits the excerpt preview required by Bible §9 (shows swatch + color label + badge + note text only). | Session 3 | Fix Now | | |
<!--
